...
function [bhat,ff,f2,r2]=rcs(x,y,knots,atwhich,plots_data,plots_spline,color)
...
if plots_data==1
    %subplot(2,1,1)
    plot(x,y,'o','MarkerSize',4,'Color',color)
elseif plots_spline==1
    hold on
    %     plot(knots,min(y)+zeros(1,length(knots)),'or') % plot knots 
    plot(gr,ff(gr),'Color',color); % plot spline fit
    %     legend('data','spline')
end

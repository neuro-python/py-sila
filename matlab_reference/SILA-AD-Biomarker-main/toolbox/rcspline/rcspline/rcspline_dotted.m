...
function [bhat,f,f2,CI,r2]=rcspline(x,y,knots,bootsams,atwhich,plots_data,plots_spline,color)
...
%       Obtained only if the bootstrap is requested and and only for the
...
n=length(y);
if nargin<6;plots_data=0;end
[bhat,f,f2,r2]=rcs(x,y,knots,atwhich,plots_data,plots_spline,color);%get the spline
...
if nargin>=4 && isempty(bootsams)~=1
    ...
    for boots=1:bootsams
        ...
        [~, fb, ~]=rcs(xb,yb,bknots,atwhich,0,0,color);
        FF(:,boots)=fb(atwhich);
    end
    ...
    for i=1:length(atwhich)
        low(i)=prctile(FF(i,:),5);
        upp(i)=prctile(FF(i,:),95);
    end
    low=low';upp=upp';
...
if plots_spline==1
    ...
    %     gr = time_range;
    % end
    plot(gr,f(gr),':','color',color, 'LineWidth', 1);hold on;
    % plot(atwhich,low,'--k');plot(atwhich,upp,'--k');
    % legend('spline', '95% CIs')
    hold off
elseif plots_spline==2
        % subplot(2,1,2)
    % figure
    % if isempty(time_range)
        gr=min(x):0.01:max(x);
    ...
    %     gr = time_range;
    % end
    plot(gr,f(gr),'color',color);hold on;
    plot(atwhich,low,'--k');plot(atwhich,upp,'--k');
    % legend('spline', '95% CIs')
    hold off
elseif plots_spline==0
